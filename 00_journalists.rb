...
def count_journalists
    puts "Il y a #{journalists.count} de journalistes"
end


#Quelle est le handle le plus court de cette liste ?
def short_name
    journalists.min {|a, b| a.lengt <=> b.lenght }
    puts "Le login le plus court c'est #{journalists.min}"
end
...
def many_maj
    puts " Il y a #{journalists.grep(/@[A-Z]/).count} login qui ont une majuscule apres le @"
end
...
def sortby_name
    puts "Voici une liste du plus petit au plus grand #{journalists.sort_by(&:length)[0...50]}"


#Quelle est la position dans l'array de la personne @epenser ?
def find_epenser
    puts "La position dans le tableau de epenser est #{journalists.find_index('@epenser')}"
end

#Repartition des logins
